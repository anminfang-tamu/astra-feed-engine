...
OrderBook *BookManager::getOrCreate(uint16_t stock_locate,
  ...
  ASTRA_TRACE("book_manager getOrCreate locate=%u channel=%u", stock_locate,
              channel_id);
  if (stock_locate == 0 || stock_locate >= kMaxStockLocate)
    return nullptr;
  ...
  const uint16_t effective_channel = locate_registered_[stock_locate]
                                         ? channel_by_locate_[stock_locate]
                                         : channel_id;
  ...
  books_[active_book_count_] = std::make_unique<OrderBook>(
      static_cast<uint32_t>(stock_locate), effective_channel, order_capacity);
  book = books_[active_book_count_].get();
  ASTRA_TRACE("book_manager created locate=%u book=%p channel=%u capacity=%zu "
              "active_before=%u",
...
void BookManager::addOrder(uint16_t stock_locate, uint64_t order_id,
                           ...
                           uint16_t channel_id) noexcept {
  ASTRA_TRACE("book_manager add locate=%u order=%llu price=%llu qty=%u side=%c "
              "channel=%u",
...
void BookManager::replaceOrder(uint16_t stock_locate, uint64_t old_id,
  ...
  ASTRA_TRACE(
      "book_manager replace locate=%u old=%llu new=%llu price=%llu qty=%u",
