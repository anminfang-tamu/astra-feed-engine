...
unsigned long envUnsigned(const char *name, unsigned long fallback,
  ...
  if (parsed < minimum || parsed > maximum) {
    throw std::runtime_error(std::string("out-of-range unsigned integer in ") +
                             name + ": " + value);
  }
...
private:
  ...
  ParsedPacket parseFast(const std::byte *frame,
    ...
    // A flow rule narrows traffic but does not guarantee a fixed frame shape
    // across PMDs. Keep the common untagged IPv4 path short, and send VLAN or
    // IPv4-option frames through the fully validated parser.
    ...
    if (total_len <
        ...
        sizeof(struct rte_ether_hdr) + static_cast<std::size_t>(total_len) >
            frame_len) {
      return {ParseStatus::Malformed, nullptr, 0, 0, true};
    }
    ...
    if (payload_size > SequencedPacket::kMaxPacketSize)
      return {ParseStatus::Malformed, nullptr, 0, 0, true};
    ...
    return {ParseStatus::Accepted, frame + payload_offset, payload_size,
            line_index, true};
  }
