...
TEST(OrderBookTest, AddOrdersUpdatesTopOfBook) {
  ...
  book.addOrder(makeMessage(1, 42, 10, 100, OrderSide::Buy,
                            MessageType::AddOrder));
  book.addOrder(makeMessage(2, 42, 11, 50, OrderSide::Buy,
                            MessageType::AddOrder));
  book.addOrder(makeMessage(3, 42, 13, 75, OrderSide::Sell,
                            MessageType::AddOrder));
  book.addOrder(makeMessage(4, 42, 12, 25, OrderSide::Sell,
                            MessageType::AddOrder));
...
TEST(OrderBookTest, ModifyOrderUpdatesBestLevelQuantity) {
  ...
  book.addOrder(makeMessage(1, 42, 10, 100, OrderSide::Buy,
                            MessageType::AddOrder));
  book.modifyOrder(makeMessage(1, 42, 10, 40, OrderSide::Buy,
...
TEST(OrderBookTest, DeleteOrderFallsBackToNextBestBid) {
  ...
  book.deleteOrder(makeMessage(2, 42, 0, 0, OrderSide::Buy,
...
TEST(OrderBookTest, DeleteOrderFallsBackToNextBestAsk) {
  ...
  book.addOrder(makeMessage(1, 42, 12, 100, OrderSide::Sell,
                            MessageType::AddOrder));
  book.addOrder(makeMessage(2, 42, 13, 50, OrderSide::Sell,
                            MessageType::AddOrder));

  book.deleteOrder(makeMessage(1, 42, 0, 0, OrderSide::Sell,
                               MessageType::DeleteOrder));
...
TEST(OrderBookTest, ModifyOrderMovesBestBidToNewPrice) {
  ...
  book.addOrder(makeMessage(1, 42, 10, 100, OrderSide::Buy,
                            MessageType::AddOrder));
  book.modifyOrder(makeMessage(1, 42, 12, 75, OrderSide::Buy,
                               MessageType::ModifyOrder));
